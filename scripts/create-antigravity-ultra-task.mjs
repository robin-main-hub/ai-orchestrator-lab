...
async function main() {
  ...
  const taskDir = join(root, taskId, "lane-a");
  ...
  console.log(`Created Ultra-first Antigravity task: ${requestFile}`);
  console.log("");
...
async function loadBody(args) {
  ...
  return [
    "Implement the assigned coding task in this isolated Ultra lane.",
    "",
    "Constraints:",
    ...
    "- Use Ultra for heavier validation when the task needs deeper confidence.",
  ].join("\n");
}
...
function createRequestMarkdown({ title, taskId, routeType, body }) {
  return [
    `# ${title}`,
    "",
    "- workerProfile: personal_antigravity_ultra",
    "- lane: lane_a",
    "- planTier: Ultra",
    "- routeType: " + routeType,
    "- taskId: " + taskId,
    ...
    "## Operating Rule",
    "",
    "You are the currently connected Ultra Antigravity/Gemini coding worker. Produce code in this isolated lane and keep the output checkpoint-friendly for Codex review and merge.",
    "",
    "## Task",
    "",
    body.trim(),
...
async function runWorker(args) {
  await new Promise((resolvePromise, rejectPromise) => {
    ...
    child.on("exit", (code) => {
      ...
      rejectPromise(new Error(`Antigravity worker dry-run failed with exit code ${code}`));
    });
  });
}

function timestampTaskId() {
  return `ultra-${new Date().toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z")}`;
}
...
function printUsage() {
  console.log(`Usage:
  node scripts/create-antigravity-ultra-task.mjs --task-id provider-refactor --title "Refactor provider registry" --body-file ./task.md

Options:
  --task-id <id>       Defaults to an ultra timestamp id.
  ...
  --run-dry-run        Also invoke run-antigravity-worker.mjs with --ultra-first --dry-run.
`);
}
