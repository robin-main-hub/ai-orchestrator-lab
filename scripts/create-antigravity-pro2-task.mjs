...
const PROFILE_ID = "personal_antigravity_pro_2";
const LANE = "lane_c";
const LANE_DIR = "lane-c";
...
async function main() {
  ...
  console.log(`Created Pro #2 Antigravity task: ${requestFile}`);
  console.log("");
...
async function loadBody(args) {
  ...
  return [
    "Implement the assigned coding task in this isolated Pro #2 lane.",
    "",
    "Constraints:",
    "- Work only on the requested files or modules.",
    "- Keep changes independent from Ultra and Pro #1 lane work.",
...
function createRequestMarkdown({ title, taskId, routeType, body }) {
  return [
    ...
    "## Operating Rule",
    "",
    "You are the Pro #2 Antigravity/Gemini coding worker. Produce code in this isolated lane and keep the output checkpoint-friendly for Codex review and merge.",
    "",
    "## Task",
    "",
    body.trim(),
...
async function runWorker(args) {
  await new Promise((resolvePromise, rejectPromise) => {
    ...
    child.on("exit", (code) => {
      ...
      rejectPromise(new Error(`Antigravity Pro #2 worker dry-run failed with exit code ${code}`));
    });
  });
}

function timestampTaskId() {
  return `pro2-${new Date().toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z")}`;
}
...
function printUsage() {
  console.log(`Usage:
  node scripts/create-antigravity-pro2-task.mjs --task-id provider-refactor-pro2 --title "Pro #2 coding task" --body-file ./task.md

Options:
  --task-id <id>       Defaults to a pro2 timestamp id.
  ...
  --run-dry-run        Also invoke run-antigravity-worker.mjs with --lane lane_c --dry-run.
`);
}
