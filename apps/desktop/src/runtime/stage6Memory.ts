...
export function createSeedMemoryRecords(createdAt: string): MemoryRecord[] {
  return [
    ...
    },
    {
      ...
      content:
        "DGX-02 is the authoritative server for shared data. MacBook keeps a local outbox and syncs when online. Home PC is treated as an online-only client.",
      sourceChannel: "desktop",
      trustLevel: "trusted",
      projectId: defaultProjectId,
      tags: ["dgx-02", "sync", "fallback"],
      activationState: "active",
      createdAt,
      lastAccessedAt: createdAt,
      pinned: true,
