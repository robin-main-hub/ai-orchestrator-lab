...
import { CircleStop, Hammer, Plus, Send, Telescope, Trash2 } from "lucide-react";
...
export function CodingWorkbench({
  ...
  const slashSuggestions = useMemo(() => {
    ...
    return SLASH_COMMANDS.filter((command) => command.name.startsWith(draft.toLowerCase())).slice(0, 8);
  }, [draft]);
  ...
  const handleSlash = async (session: CodingSession, raw: string): Promise<boolean> => {
    ...
    switch (command.kind) {
      ...
      case "undo":
        ...
        setNotice("마지막 턴을 되돌렸습니다.");
        break;
      case "clear":
        patchSession(session.id, (current) => ({ ...current, messages: [], checkpoints: [], compactedSummary: undefined }));
        break;
      case "share":
        try {
      ...
      case "build":
        ...
        break;
      case "init":
        void runTurn(session, "이 저장소를 조사해서 (read/grep/glob 사용) AGENTS.md 초안을 제안해줘. 빌드/테스트 명령과 컨벤션을 포함해서.");
        break;
      case "help":
