...
export function createExperienceRoadmap({
  ...
  return [
    ...
    item("receipt_ledger", "작업 영수증 장부", "PR, 테스트, 실패, 수정, 마스킹 상태가 한 장의 카드로 남음", "cline", hasReceipts ? "live" : "next"),
    ...
    item("task_graph", "Issue-like 작업 그래프", "대화, 토론, tmux 실행, 승인, 영수증을 같은 작업 id로 묶음", "linear", hasHandoffs ? "live" : "next"),
    ...
    item("attachment_pipeline", "첨부 처리 파이프라인", "이미지/문서/텍스트 첨부를 모델 능력과 공개 영수증에 맞춰 일원화", "cursor", readyMaturityIds.has("08_attachments") ? "live" : "next"),
