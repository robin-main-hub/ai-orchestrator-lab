...
- **대화 워크벤치 OpenCode급 업그레이드** (2026-06-11, 14항목) — `대화` 탭이 코딩 워크벤치와 같은 메커니즘으로 동작:
  ...
  - 실사용 안정화(동일자 후속): 응답 상한 512→4096(`maxOutputTokens` rider, MiMo `max_completion_tokens` 포함) + 완성 타임아웃 120초 — DGX-02 실호출로 4096토큰 출력 검증; SSE 스트림 정체 감시(연결 15초/무활동 90초 → 비스트림 폴백); 도구 승인 대기 시 인라인 카드 자동 표시 + 버블에 "⏳ 승인 대기" 노출; 에이전트가 확인 질문을 던지면 추천 칩이 "보낼 수 있는 답변" 3개로 전환(표 안 질문 인식)
...
73     example-domain-workflow-templates (HTV quote/material research/sample request templates + 4-6 core hermes org) [PR7]
```
...
- DGX-02 모델 레지스트리는 `qwen36-domain-lora-v5-prisma`를 노출한다.
