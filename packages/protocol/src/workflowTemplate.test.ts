...
describe("core workflow templates (Generic App/Design Builder)", () => {
  ...
  it("carries NO company/business strings or domains in the core registry", () => {
    ...
    for (const banned of ["giolite", "GIOLITE", "HTV", "견적", "샘플", "거래처"]) {
...
describe("business templates are removed from the product", () => {
  it("the GIOLITE business templates are no longer reachable by any id", () => {
    expect(findWorkflowTemplate("giolite_htv_quote")).toBeUndefined();
    expect(findWorkflowTemplate("giolite_material_research")).toBeUndefined();
    expect(findWorkflowTemplate("giolite_sample_request")).toBeUndefined();
    // 코어 generic 템플릿만 남음
