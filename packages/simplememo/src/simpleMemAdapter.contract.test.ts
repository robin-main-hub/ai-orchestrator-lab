...
describe("withTrustEnforcement(SimpleMemAdapter) — integration", () => {
  ...
  it("gates batchRemember based on trust policy", async () => {
    const adapter = withTrustEnforcement(new SimpleMemAdapter());
    const ctx = makeContractCtx({
    ...
    await expect(
      ...
      new MemoryAdapterError("trust_violation", "Untrusted memories require curator promotion."),
    );
  });
});

describe("SimpleMemAdapter — Redis scan cap, HNSW configuration, and batchRemember", () => {
  it("enforces Redis scan capping with warning on recall", async () => {
    const seedRecords: MemoryRecord[] = Array.from({ length: 25 }, (_, i) => ({
      id: `dgx_rec_${i}`,
      layer: "episode",
      scope: "session",
      kind: "context",
      title: `record ${i}`,
      content: `content ${i} about apple`,
      sourceChannel: "agent",
      trustLevel: "trusted",
      createdAt: new Date().toISOString(),
      pinned: false,
    ...
    // scanBatchSize = 1 means 20 rounds can only check 20 records (out of 25)
    const adapter = new SimpleMemAdapter({ seedRecords, scanBatchSize: 1 });
    const events: EventEnvelope<any>[] = [];
    const ctx = makeContractCtx({
      appendEvent: async (ev) => {
        events.push(ev);
    ...
    const results = await adapter.recall({ query: "apple" }, ctx);
    // Since it's sorted by score and they all match, it should return up to the limit (8),
    // but the key point is we checked the cap, returning partial records and a warning.
    expect(results.length).toBeLessThanOrEqual(8);
    expect(results[0]?.reason).toContain("warning: Scan cap reached");

    const recallEvent = events.find((e) => e.type === "memory.operation" && e.payload.operation === "recall");
    expect(recallEvent).toBeDefined();
    expect(recallEvent?.payload.warning).toContain("Scan cap reached");
  });

  it("exposes HNSW stats configuration fields", async () => {
    // 1. forceHnsw=false -> configured
    const a1 = new SimpleMemAdapter({ forceHnsw: false });
    const s1 = await a1.stats();
    expect((s1 as any).hnswStatus).toBe("configured");

    // 2. forceHnsw=true, hnswSupported=false -> blocked
    const a2 = new SimpleMemAdapter({ forceHnsw: true, hnswSupported: false });
    const s2 = await a2.stats();
    expect((s2 as any).hnswStatus).toBe("blocked");

    // 3. forceHnsw=true, hnswSupported=true -> active
    const a3 = new SimpleMemAdapter({ forceHnsw: true, hnswSupported: true, hnswObserved: true });
    const s3 = await a3.stats();
    expect((s3 as any).hnswStatus).toBe("active");
    expect((s3 as any).hnswObserved).toBe(true);
  });

  it("performs async-simulated batchRemember with events and fallback recall support", async () => {
    const adapter = new SimpleMemAdapter();
    const events: EventEnvelope<any>[] = [];
    const ctx = makeContractCtx({
      appendEvent: async (ev) => {
    ...
    const result = await adapter.batchRemember!(
      [
        {
          title: "success 1",
    ...
    expect(job.accepted).toBe(1);
    expect(job.rejected).toBe(1);
    expect(job.itemResults).toHaveLength(2);
    expect(job.itemResults[1]?.status).toBe("rejected");

    // Verify all 3 events are appended
    const acceptedEvent = events.find((e) => e.type === "memory.batch.accepted");
    const startedEvent = events.find((e) => e.type === "memory.batch.started");
    const partialEvent = events.find((e) => e.type === "memory.batch.partial");

    expect(acceptedEvent).toBeDefined();
    expect(startedEvent).toBeDefined();
    expect(partialEvent).toBeDefined();

    // Verify successful memory is recallable
    const recallResults = await adapter.recall({ query: "success" }, ctx);
    expect(recallResults).toHaveLength(1);
    expect(recallResults[0]?.record.title).toBe("success 1");
  });
});
