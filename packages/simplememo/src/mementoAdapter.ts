...
import { memoryBatchRememberOptionsSchema } from "@ai-orchestrator/protocol";
import type { MemoryAdapter, MemoryAdapterContext, MemoryAdapterKind, MemoryBatchRememberResult } from "./adapter.js";
...
export class MementoMcpAdapter implements MemoryAdapter {
  ...
  async batchRemember(
    ...
    const now = ctx.now?.() ?? this.nowFn();
    const jobId = `memento_batch_${stableId(inputs.map(i => i.title).join(",") + now)}`;
    ...
    for (let i = 0; i < inputs.length; i++) {
      ...
      if (!input.title || input.title.trim() === "" || !input.content || input.content.trim() === "") {
        rejected++;
        errors.push({ itemIndex: i, error: `Invalid input` });
        itemResults.push({
        ...
        accepted++;
        const record: MemoryRecord = {
          id: `memento_${stableId(`${input.title}:${input.content}:${now}:${i}`)}`,
          layer: input.layer,
          scope: input.scope ?? inferScope(input.layer),
          kind: input.kind ?? "context",
          title: input.title,
    ...
    await ctx.appendEvent?.({
      id: `${jobId}_finished`,
      sessionId: "memento_mcp",
      type: `memory.batch.${status}` as any,
      payload: {
        kind: `memory_batch_${status}`,
        jobId,
        idempotencyKey,
