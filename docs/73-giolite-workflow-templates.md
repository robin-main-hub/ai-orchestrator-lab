# 73 — GIOLITE workflow templates + 핵심 페르소나 조직 (Orchestration OS PR7)

이 시스템이 코딩만 하는 게 아니라 회사 업무(GIOLITE)에도 바로 쓰이게. UI가 아니라
...
outputArtifacts) + `WorkflowInputField`. 3개 GIOLITE 프리셋:
...
· 코코아/Mediator(merge_recommend) · GIO/Sales(external, research) · Memory Curator
