...
## §0-C 어휘 확정: "영수증" 폐지 → "브리핑" (2026-07-10 사용자 확정)

전 UI에서 "영수증" 계열 명칭을 다음 매핑으로 일괄 교체한다(WORD-1 슬라이스):
- "작업 영수증" → **"작업 브리핑"** / 단독 "영수증" → **"브리핑"**
- "토론 공개 영수증"·"공개 영수증" → **"공개 브리핑"**
- "작업 영수증 장부"·"영수증 장부" → **"브리핑 로그"**
적용 범위: 표시 문자열·aria-label·힌트/설명·명령팔레트 라벨·카피 핀 테스트. **코드 식별자(WorkReceipt* 등)는 유지**(불필요 churn 금지). gio-erp의 "90초 브리핑 금지" 규칙은 별개 도메인이라 무관.
