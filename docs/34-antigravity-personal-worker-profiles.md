...
scripts/create-antigravity-pro2-task.mjs
...
After Pro #1 is verified, prepare Pro #2 on `lane_c`. It should also be a coding worker, with assigned files or modules that do not overlap with active Ultra or Pro #1 work.
...
Pro #2 task bootstrap after Pro #1 is stable:

```powershell
$env:ENABLE_PERSONAL_ANTIGRAVITY_PROFILES="true"
$env:OWNER_USER_ID="robin"
node scripts/create-antigravity-pro2-task.mjs --task-id first-pro2-task --title "First Pro #2 coding task" --body "Implement this task in the Pro #2 lane." --run-dry-run
```
