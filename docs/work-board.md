...
origin/main @ `310e38d6` (`#188` merge). R4 v0 cascade train + R5 Stage 1b primitive adoption 1차/2차 wave 완료 (2026-05-26 sync, origin/main @ `310e38d6`; 후속 `#156`~`#188`). 아래 R4-era open PR 표와 "Stage 1b 진행 중" 표현은 이 섹션이 최신 기준으로 supersede한다.
...
- `EvolveMementoPanel` RecordChips inline tag - TODO / safe mechanical
- `CommandPalette` - TODO / design judgment needed. v0 CommandPalette 구조와 현재 repo verb-command 구조가 달라 단순 치환 금지.
- `AvatarWithStatus` adoption - TODO / design judgment needed. primitive 는 있으나 `AgentAvatar` 는 protocol-aware component 이므로 무단 교체 금지.
...
마지막 갱신: 2026-05-26 라운드 **5** (Stage 1b primitive adoption 1차/2차 wave: `#181`~`#188` merge 상태 반영. R4-era 표와 표현은 위 R5 canonical update 가 supersede).
