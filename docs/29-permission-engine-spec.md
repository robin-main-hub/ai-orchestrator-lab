...
- Mobile/Telegram에서 들어온 요청에 대한 통합 evaluator 0.
...
```
1. Inbound request          (provider call / remote run / mobile / telegram / tmux dispatch / external send)
       │
...
4. Enqueue ApprovalQueueItem (state: required) + notify channels
   (desktop drawer / mobile push / telegram message)
       │
...
기본 원칙:
- **untrusted source** (telegram, api, mobile guest) → memory recall 차단, 모든 effect는 approval_required 또는 deny.
...
### 6.7 future `/ingress/telegram`, `/ingress/api`
...
push notification (iOS PWA는 16.4+ 제한적) 또는 telegram bot으로 알림 — 어느 쪽이든 server `/approvals/notify` endpoint가 보냄.
...
| F9 | Ingress receiver (Telegram / mobile webhook) + IngressGuardResult 7단계 | F4 + F7 |
...
3. **2FA 메커니즘**: 모바일 push + 코드 vs telegram bot inline button. (Claude 추천: 모바일 PWA + telegram 둘 다 보내고 어느 쪽이든 응답 받으면 grant)
