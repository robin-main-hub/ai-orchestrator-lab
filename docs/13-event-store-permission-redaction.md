...
AI Orchestrator Lab의 모든 대화, 토론, 실행, 백업, 모바일 승인, 외부 인입 브리지 이벤트는 하나의 Event Store 계열을 기준으로 기록된다. 중앙 권위는 DGX-02 Event Store이며, MacBook과 집 PC는 로컬 cache/outbox를 가진 client replica로 동작한다. Obsidian, Notion, 모바일 대시보드, SimpleMem index는 모두 projection이다.
...
- External Ingress, 모바일, API에서 들어온 위험 명령은 기본적으로 `pending approval`이다.
...
export const EventEnvelopeSchema = z.object({
  ...
  source: z.enum(["desktop", "server", "external_legacy", "mobile", "api", "system"]),
...
| External Ingress | untrusted | 외부 채널에서 들어온 메시지. 자동 실행/자동 recall 제한 |
...
| External Ingress | allowed | pending | pending | pending | denied | denied |
...
- external, mobile, and external API events must never go directly to `tmux send-keys`; they must become permissioned command intents first.
...
외부 인입에서 온 명령은 파일 쓰기, 터미널 실행, 네트워크 호출을 모두 pending으로 둔다.
...
9. external/mobile approval event 연결
...
- 외부 인입/OpenClaw 등 외부 채널에서 온 실행성 요청은 `external_channel` actor로 기록한다.
