...
streaming, tool use, multimodal, mobile approval, 외부 인입은 슬라이스 1 비포함. 슬라이스 닫힌 뒤 별도 phase.
...
| 항목 | 성공 | 실패 |
|---|---|---|
| Outbox enqueue | Desktop app writes events to Stage29 `LocalClientEventCache`, the single client cache/outbox, then pushes batches to DGX-02 | Any revived Stage16 path or duplicate outbox source |
...
| Outbox merge | Stage29 merges unsynced local events and server replay events with one dedupe rule; projected events must not re-enter the unsynced queue | Re-appending projected events makes them unsynced again |
