...
- **격리**: GIOLITE(HTV 견적/반사소재 조사/샘플 요청)를 `packages/protocol/src/domainPacks/
...
- 코어 registry에 `giolite/HTV/견적/샘플/거래처` 문자열·`sales/research/sample` 도메인 **0**.
- 기본 `findWorkflowTemplate("giolite_htv_quote")` → undefined(격리). 명시적으로 합친
  registry에서만 보임.
