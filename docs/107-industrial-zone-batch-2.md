...
| PR | merge | 라인 | 내용 |
|---|---|---|---|
| #546 | `cb7818b` | A | OS generic evidence plugin boundary (p61-p80 audit, docs, GIO 예제 분리) — 병렬 워커 |
...
- `smokeIngest.ts`(GIO-flavored) → `smokeGenericEvidenceIngest.ts`(neutral) + `examples/giolite-erp-plugin/`로 도메인 예제 분리.
