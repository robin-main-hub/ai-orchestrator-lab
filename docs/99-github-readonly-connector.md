...
## D2 컨텍스트 attach (사용자 선택 PR → 코딩 컨텍스트)

D1에서 본 PR을 **사용자가 명시적으로 선택**해 코딩 세션 컨텍스트에 붙인다. 자동 주입 없음.

- PR 상세의 "컨텍스트에 추가" → 워크벤치가 서버로 PR을 **다시 GET**(observed 재확인, 클라이언트가
  들고 있던 내용을 믿지 않음) → `GithubContextAttachment` 생성 → 세션 `githubContext`에 upsert.
- 본문은 **결정적 bounded excerpt**(원본 raw 전체 아님) + `truncated` 플래그. LLM 요약 없음
  (`summarySource: "github_observed"`, `truthStatus: "observed"`).
- 중복 attach는 idempotent(같은 repo+PR번호는 한 번만).
- 코딩 **첫 요청에만** 주입(`maxItems` 5 · `maxChars` 12000), 제외된 개수를 정직하게 표시 —
  tool 라운드마다 반복 주입하지 않아 토큰 폭증 방지.
- attach 시 `coding.github.context.attached` trace 이벤트(repo/number/title/url/observedAt만 —
  본문 excerpt·토큰·헤더 제외, private raw body 미저장).

코드: `protocol/githubConnector`(GithubContextAttachment), `lib/githubContext`(빌더·idempotent
upsert·주입), `coding/GithubPullRequestPanel`(추가 버튼·첨부 칩), `CodingWorkbench`(attach 핸들러·
첫 요청 주입), `CodingSession.githubContext`.

## D3 GitHub read-only MCP tools

기존 `/integrations/github` 라우트를 MCP 도구로 노출 — 에이전트가 read-only로 직접 조회.
새 GitHub client를 만들지 않고 기존 service/route를 그대로 forward한다(`packages/mcp`의
swarm 도구가 orchestrator HTTP 라우트로 forward하는 패턴 재사용).

| MCP 도구 | forward 대상 (GET) |
|---|---|
| `github_status` | `/integrations/github/status` |
| `github_pr_list` | `/repos/:owner/:repo/pulls` |
| `github_pr_read` | `/repos/:owner/:repo/pulls/:number` |
| `github_file_read` | `/repos/:owner/:repo/file?path=…` (신규 file 라우트) |

원칙:
- **읽기 전용 by construction**: 모든 github 도구는 GET, 쓰기 도구 0.
- **토큰 격리**: GitHub 토큰은 서버 env에만. MCP→서버는 orchestrator 토큰으로 인증하고,
  GitHub 토큰을 다루지 않는다. 에러 메시지는 토큰 마스킹.
- **bounded + 정직 outcome**: 결과는 라우트가 주는 그대로(observed/not_configured/
  permission_denied/connection_failed). `github_file_read`는 24K로 자르고 `truncated` 표시
  (raw 전체 미반환).
- **자동 attach 금지**: 에이전트가 도구 결과를 읽을 뿐, MCP 레이어가 Mission/Coding
  컨텍스트에 자동으로 붙이지 않는다(attach는 D2의 사용자 명시 액션 유지).
- **path injection 방지**: owner/repo는 `[A-Za-z0-9._-]`만 허용+인코딩, 잘못된 인자는
  fetch 없이 깨끗한 에러.

코드: `packages/mcp/src/swarmTools.ts`(github_* 도구 + 경로 검증), `apps/server`의 file
라우트 + `getFileContent`(base64 디코드·bounded), `protocol`의 GithubFileContent.

## 다음 단계 (이번 범위 밖)

- D4: GitHub write actions safety **design doc만**(`docs/github-write-actions-safety.md`) —
  repo allowlist·branch policy·diff preview·approval·dry-run·rollback. 구현은 그 이후.
- code_search / issue_read 도구(결과량·prompt 폭증 고려해 후순위).
